...
//        product.getParts().addAll(assparts);
 ...
 //        return "confirmationassocpart";
    ...
    public String buyProduct(@RequestParam("productID") int theId) {
        ProductService productService = context.getBean(ProductServiceImpl.class);
        product1 = productService.findById(theId);

        int inventory = product1.getInv();

        if (inventory >= 1) {
            int decrementInv = inventory - 1;

            product1.setInv(decrementInv);
            productService.save(product1);

            return "boughtproductsuccessfully";
        } else {
            return "boughtproducterror";
        }
    }
}
